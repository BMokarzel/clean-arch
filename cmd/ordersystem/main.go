...
import (
	...
	"github.com/BMokarzel/clean-arch.git/internal/event/handler"
	"github.com/BMokarzel/clean-arch.git/internal/infra/database"
	"github.com/BMokarzel/clean-arch.git/internal/infra/graph"
...
func main() {
	...
	err = orderRepository.Migrate()
	if err != nil {
		log.Fatalf("Error to create tables. Error: %s", err)
	}
	createOrderUseCase := usecase.NewCreateOrderUseCase(orderRepository, event.NewOrderCreated(), eventDispatcher)
	listOrderUseCase := usecase.NewListOrderUseCase(orderRepository, event.NewOrderListed(), eventDispatcher)

	webserver := webserver.NewWebServer(configs.WebServerPort)
	webOrderHandler := web.NewWebOrderHandler(eventDispatcher, orderRepository, createOrderUseCase.OrderCreated, listOrderUseCase.OrderListed)
