...
func (s *OrderService) ListOrder(ctx context.Context, empty *emptypb.Empty) (*pb.ListOrderResponse, error) {
	ordersList, err := s.ListOrderUseCase.Execute()
	if err != nil {
		return nil, err
	}

	orders := make([]*pb.Order, 0, len(ordersList))
